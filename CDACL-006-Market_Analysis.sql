...
SELECT order_id, AVG(product_id) AS avg_products_per_order
FROM order_products_train
GROUP BY order_id;

## 12. What are the most reordered products in each department?
SELECT p.department_id, opt.product_id, 
SUM(opt.reordered) AS reordered_products
FROM products p JOIN order_products_train opt
ON p.product_id = opt.product_id
...
SELECT order_id, COUNT(*) AS no_of_products_per_order
FROM order_products_train
GROUP BY order_id;

## 17. What is the average reorder rate for products in each aisle?
SELECT p.aisle_id, AVG(opt.reordered) AS avg_reorder_rate
...
SELECT o.order_dow AS order_day, AVG(order_number) AS avg_order_size
FROM order_products_train opt
JOIN orders o ON opt.order_id = o.order_id
...
SELECT aisle_id, department_id, COUNT(*) AS product_count
FROM products 
